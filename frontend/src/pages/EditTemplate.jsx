...
const EditTemplate = () => {
  ...
  const loadTemplate = async () => {
    try {
      ...
      console.log('Edit Template API response:', response.data);
      ...
      if (response.data.success && response.data.data && response.data.data.template) {
        ...
        console.log('Template data for edit (direct):', template);
        setFormData({
          name: template.name,
          subject: template.subject,
          htmlBody: template.htmlBody,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await templateAPI.update(name, templateData);
      toast.success('Template updated successfully in AWS SES!');
      navigate('/templates');
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to update template in SES');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-4xl font-bold text-foreground">Edit Email Template</h1>
          <p className="text-muted mt-2">Update your email template in AWS SES</p>
        </div>
      </div>
          ...
          <div className="space-y-4">
            <div className="form-group">
              ...
              <input
                ...
                disabled
              />
              <p className="text-sm text-muted mt-1">Template name cannot be changed in AWS SES</p>
            </div>
            <div className="form-group">
