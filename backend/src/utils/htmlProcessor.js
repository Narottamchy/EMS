...
async function validateHTML(htmlContent) {
  try {
    ...
    logger.info(`HTML validation completed: ${isValid ? 'Valid' : 'Invalid'} (${errors.length} errors, ${warnings.length} warnings)`);
 ...
 */
async function generateTextFromHTML(htmlContent) {
  try {
    if (!htmlContent.trim()) {
      return '';
    }
    ...
    logger.info('✅ HTML converted to text successfully');
...
function minifyHTML(htmlContent) {
  try {
    ...
    logger.info('✅ HTML minified successfully');
    return cleaned;

  } catch (error) {
    logger.error('❌ HTML minification error:', error);
